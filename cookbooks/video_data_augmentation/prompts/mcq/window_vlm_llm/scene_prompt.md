...
You are analyzing traffic surveillance footage to produce a **structured scene description** that will later be mapped to the fixed multiple-choice question bank in `cookbooks/video_data_augmentation/question_bank.json`.
...
The labels below are evidence cues for the downstream mapper, not final MCQ answers. Do not output MCQ JSON; the mapper will convert your evidence into exact option strings from `cookbooks/video_data_augmentation/question_bank.json`.
