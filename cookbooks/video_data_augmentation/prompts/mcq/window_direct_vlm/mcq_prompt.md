...
**Purpose:** Convert the structured scene description into a strict MCQ JSON answering the question bank in `cookbooks/video_data_augmentation/question_bank.json`.

**Synchronization note:** The decision rules below embed traffic-domain knowledge (accident-timing logic, pedestrian definition, weather/wet-road cues) that isn't expressible in the question bank JSON itself; the question list is injected at `{question_bank_json}` below so it always mirrors the canonical bank. This mode keeps a separate prompt file so `window-direct-vlm` can evolve independently if its direct VLM answering instructions diverge from the other MCQ modes.
