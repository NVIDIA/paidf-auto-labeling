...
For each window of ``<clip_path>`` the runner samples frames, asks a VLM to
caption the scene, then prompts an LLM to produce MCQ/BCQ task items from that
caption. Items are aggregated across windows, handed to the task converter for
the final output files, and companion artifacts (window metadata, verifier
...
def setup_logger(verbose: bool) -> logging.Logger:
    return setup_runner_logger("window_vlm_llm", verbose)


@dataclass
class WindowVlmLlmRunner:
    scene_prompt: str
    mcq_prompt: str
    vlm_base_url: str
    vlm_model: str
    ...
    # Retry missing question ids (best-effort, per-window) and merge into the MCQ JSON.
    retry_missing_questions: bool = False
    retry_missing_max_rounds: int = 2
    # Optional: keep a copy of the question bank for retry prompt subsets.
    question_bank: Optional[Dict[str, Any]] = None
    # Optional post-check: verify finalized per-window MCQ with VLM frames.
    ...
    def _verify_window_mcq(
        ...
        *,
        ...
        return run_window_vlm_verify(
            ...
            retry_stage="window_vlm_llm:vlm_verify",
            logger=logger,
        ...
        try:
            ...
            if wf > 0:
                ...
                for w_idx, (start_frame, end_frame) in enumerate(win_iter):
                    ...
                    caption = ""
                    if frames:
                        messages = vlm_messages_from_frames(frames, self.scene_prompt)
                        try:
                            caption = call_chat_raw(
                                ...
                                retries=int(self.vlm_retries or 0),
                                retry_backoff_s=float(self.vlm_retry_backoff_s or 5.0),
                                retry_stage="window_vlm_llm:vlm_caption",
                                api_key=get_vlm_api_key(),
                            ).strip()
                        except Exception as e:
                            win_errors.append(f"vlm_call_failed:{e.__class__.__name__}")
                            logger.exception("VLM call failed (clip=%s window=%d): %s", video_id, w_idx, e)
                            caption = ""
                    ...
                    if caption:
                        try:
                            llm_messages = [
                                {"role": "system", "content": self.mcq_prompt},
                            ...
                            mcq_obj, mcq_text = call_chat_json_with_structured_fallback(
                                ...
                                retry_stage="window_vlm_llm:llm_mcq",
                                api_key=get_llm_api_key(),
                            )
                        except Exception as e:
                            win_errors.append(f"llm_call_failed:{e.__class__.__name__}")
                            logger.exception("LLM MCQ call failed (clip=%s window=%d): %s", video_id, w_idx, e)
                            mcq_text = ""

                    if bool(self.retry_missing_questions) and caption:
                        ...
                        if bank is not None:
                            ...
                            for attempt in range(max(1, int(self.retry_missing_max_rounds or 1))):
                                required_missing = _missing_expected_ids(
                                    bank=bank,
                                    include_if_map=self.include_if_map,
                                    mcq_obj=cur_obj,
                                )
                                ...
                                retried = retry_fill_missing_questions(
                                    ...
                                    retry_stage="window_vlm_llm:llm_mcq:retry_required",
                                    max_rounds=1,
                                    logger=logger,
                                )
                                if not isinstance(retried, dict):
                                    break
                                after = set(present_ids(retried))
                                filled = sorted(after - before)
                                cur_obj = retried
                                remain = _missing_expected_ids(
                                    bank=bank,
                                    include_if_map=self.include_if_map,
                                    mcq_obj=cur_obj,
                                )
                    ...
                    if isinstance(mcq_obj, dict):
                        ...
                        # LLM call returned without raising, but did not yield parseable MCQ JSON.
                        ...
                        win_errors.append(f"llm_mcq_not_parseable:{reason}")
                        logger.warning(
                            "LLM MCQ output not parseable (clip=%s window=%d reason=%s has_output=%s output_chars=%d)",
                            video_id,
                            w_idx,
                            reason,
                ...
                for w_idx, (start_sec, end_sec) in enumerate(iter_windows(info.duration_sec, ws)):
                    ...
                    if frames:
                        ...
                        try:
                            ...
                            win_errors2.append(f"vlm_call_failed:{e.__class__.__name__}")
                            logger.exception("VLM call failed (clip=%s window=%d): %s", video_id, w_idx, e)
                            caption = ""
                    ...
                    if caption:
                        try:
                            ...
                            win_errors2.append(f"llm_call_failed:{e.__class__.__name__}")
                            logger.exception("LLM MCQ call failed (clip=%s window=%d): %s", video_id, w_idx, e)
                            mcq_text = ""
                    ...
                    if isinstance(mcq_obj, dict):
                        ...
                        # Do not log model output contents; prompts/responses can contain user-controlled data.
                        reason = classify_mcq_json_parse_failure(mcq_text)
                        win_errors2.append(f"llm_mcq_not_parseable:{reason}")
                        logger.warning(
                            "LLM MCQ output not parseable (clip=%s window=%d reason=%s has_output=%s output_chars=%d)",
                            video_id,
                            w_idx,
            ...
            # Record windowing + frame sampling parameters for reproducibility
            # and downstream metadata-llm reuse without re-running caption generation.
            metadata["windowing"] = {
            ...
            metadata["valid"] = True
            metadata["has_caption"] = any(bool(w.get(cap_key)) for w in windows_out)
            ...
            logger.info(
                "window-vlm-llm: metadata sidecar written (windows=%d enhanced_payload_objects=%d verify_enabled=%s)",
                len(windows_out),
                enhanced_objects,
            ...
            if bool(self.vlm_verify_enabled) and mcq_written and verify_windows_out:
                ...
                write_json(sidecar_verify, verify_sidecar, sanitize_paths=True)
