...
def strip_ungrounded_id_annotations(text: Any, suffix_to_key: dict[str, str]) -> Any:
    ...
    def _replace(m: re.Match[str]) -> str:
        if any(
            _canonical_numeric_suffix(s) in suffix_to_key
            for s in re.findall(r"\d+", m.group(0), timeout=re_search_timeout())
        ):
            return str(m.group(0))
        return ""

    replaced = _ID_ANNOTATION_RE.sub(_replace, text, timeout=re_search_timeout())
    return str(replaced).lstrip()
