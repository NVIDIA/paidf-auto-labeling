...
RE_SEARCH_TIMEOUT_ENV = "RE_SEARCH_TIMEOUT"
...
def re_search_timeout() -> float | None:
    """Return the shared regex timeout, matching MCQ/VLM JSON parsing behavior."""
    try:
        timeout_s = float(os.environ.get(RE_SEARCH_TIMEOUT_ENV, "60"))
...
def match_letter_prefix(text: str) -> re.Match[str] | None:
    try:
        return LETTER_PREFIX_RE.match(text, timeout=re_search_timeout())
    except TimeoutError:
        return None
