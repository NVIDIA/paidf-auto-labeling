...
from reasoning.contextual import to_daft_events, to_daft_image, to_daft_video
...
# One ctx per scene flavor — built once so each test case is a one-line call.
...
# Convenience alias for tests merged from a branch where a bare ``video_id=``
# kwarg was passed to converters; equals ``VIDEO_CTX.media_id``.
VIDEO_ID = VIDEO_CTX.media_id


def pl_video_dict(**overrides) -> dict:
    # Shape PL's VLM runner produces after `_normalize_video_json_obj`.
    base = {
        "video_id": "whatever-the-vlm-said",  # converter overrides via ctx
        "format": "mp4",
        "fps": 29.97,
        "duration": 30.5,
        "height": 720,
...
class TestVideo:
    def test_happy_path_only_allowed_keys(self):
        ...
        # scene-canonical one from ctx.
        out = to_daft_video(pl_video_dict(), ctx=VIDEO_CTX)
        assert set(out.keys()) == {
            "version",
            "video_id",
            "format",
    ...
    def test_image_ctx_rejected(self):
        with pytest.raises(DaftConvertError, match="image SceneContext"):
            to_daft_video(pl_video_dict(), ctx=IMAGE_CTX)

    def test_fps_float_rounded_to_int(self):
        assert to_daft_video(pl_video_dict(fps=29.97), ctx=VIDEO_CTX)["fps"] == 30
        assert to_daft_video(pl_video_dict(fps=24.0), ctx=VIDEO_CTX)["fps"] == 24

    def test_height_width_accept_whole_floats(self):
        # VLM occasionally emits 720.0 instead of 720.
        out = to_daft_video(pl_video_dict(height=720.0, width=1280.0), ctx=VIDEO_CTX)
    ...
    def test_missing_required_field_raises(self, missing):
        ...
        with pytest.raises(DaftConvertError, match=missing):
            to_daft_video(d, ctx=VIDEO_CTX)


class TestEvents:
    def test_happy_path(self):
        out = to_daft_events({"events": [pl_event()]}, ctx=VIDEO_CTX, duration=10.0)
        assert out["version"] == DAFT_VERSION
        assert out["video_id"] == VIDEO_CTX.media_id
        assert out["metadata"]["type"] == "events"
        [ev] = out["events"]
        assert TIMECODE_RE.fullmatch(ev["start_time"], timeout=re_search_timeout())
        assert TIMECODE_RE.fullmatch(ev["end_time"], timeout=re_search_timeout())
        assert ev["event_id"] == "e1"
        assert ev["event_caption"] == "Vehicle A rear-ends vehicle B."

    def test_image_ctx_rejected(self):
        with pytest.raises(DaftConvertError, match="image scenes"):
            to_daft_events([pl_event()], ctx=IMAGE_CTX)

    def test_accepts_bare_list(self):
        out = to_daft_events([pl_event()], ctx=VIDEO_CTX)
        assert len(out["events"]) == 1

    def test_dict_events_must_be_list(self):
        with pytest.raises(DaftConvertError, match="'events' must be a list"):
            to_daft_events({"events": "bad"}, ctx=VIDEO_CTX)

    def test_malformed_event_is_skipped(self, caplog):
        out = to_daft_events(
            {"events": [pl_event(event_id="good"), {"start_time": 0.0, "end_time": 1.0}]},
            ctx=VIDEO_CTX,
        )
        assert [ev["event_id"] for ev in out["events"]] == ["good"]
        assert "skipping malformed event" in caplog.text

    def test_empty_list(self):
        out = to_daft_events([], ctx=VIDEO_CTX)
        assert out["events"] == []

    def test_seconds_converted_to_timecodes(self):
        out = to_daft_events([pl_event(start_time=0.0, end_time=95.5)], ctx=VIDEO_CTX)
        [ev] = out["events"]
    ...
    def test_clamping_to_duration(self):
        ...
        out = to_daft_events(
            [pl_event(start_time=-1.0, end_time=999.0)],
            ctx=VIDEO_CTX,
            duration=10.0,
        )
        [ev] = out["events"]
    ...
    def test_reversed_time_range_skipped(self, caplog):
        """Detector quirks (swapped fields, end-clamp-to-zero on truncated tails)
        must not propagate downstream as reversed timecodes. The converter
        drops malformed per-event entries and preserves the rest of the file.
        """
        out = to_daft_events(
            [pl_event(start_time=5.0, end_time=2.0)],
            ctx=VIDEO_CTX,
            duration=10.0,
        )
        assert out["events"] == []
        assert "earlier than start_time" in caplog.text

    def test_clamp_then_monotonicity_check(self):
        """Clamping is applied first; the monotonicity check sees post-clamp
        values. A negative start clamps to 0 and a positive end stays put,
        which must remain valid (regression guard against an over-zealous
        check that read the raw values pre-clamp)."""
        out = to_daft_events(
            [pl_event(start_time=-1.0, end_time=2.0)],
            ctx=VIDEO_CTX,
            duration=10.0,
        )
        [ev] = out["events"]
        assert ev["start_time"] == "00:00"
        assert ev["end_time"] == "00:02"

    def test_equal_start_and_end_allowed(self):
        """Zero-duration events (instantaneous markers, e.g. a "moment of
        impact" tag) are valid — only ``end < start`` is rejected."""
        out = to_daft_events(
            [pl_event(start_time=2.5, end_time=2.5)],
            ctx=VIDEO_CTX,
            duration=10.0,
        )
        [ev] = out["events"]
        assert ev["start_time"] == ev["end_time"] == "00:02.500"
    ...
    def test_strips_per_event_extras(self):
        ev_in = pl_event(_debug="trace", confidence=0.92)
        out = to_daft_events([ev_in], ctx=VIDEO_CTX)
        [ev] = out["events"]
    ...
    def test_accepts_string_timecodes_too(self):
        ...
        ev_in = pl_event(start_time="00:01.500", end_time="00:04.500")
        out = to_daft_events([ev_in], ctx=VIDEO_CTX)
        [ev] = out["events"]
    ...
    def test_missing_required_field_skipped(self, missing, caplog):
        ev_in = pl_event()
        del ev_in[missing]
        out = to_daft_events([ev_in], ctx=VIDEO_CTX)
        assert out["events"] == []
    ...
    def test_bad_timestamp_skipped(self, caplog):
        out = to_daft_events([pl_event(start_time="nope")], ctx=VIDEO_CTX)
        assert out["events"] == []
        assert "start_time" in caplog.text

    def test_instances_source_threaded_from_ctx(self):
        # v3 multi-source hook: when ctx carries an instances_source, every
        # events.json carries the same string at top level.
        ctx = SceneContext(media_id="main", instances_source="instances_auto.json")
        out = to_daft_events([pl_event()], ctx=ctx)
        assert out["instances_source"] == "instances_auto.json"

    def test_no_instances_source_by_default(self):
        out = to_daft_events([pl_event()], ctx=VIDEO_CTX)
        assert "instances_source" not in out
...
class TestEventsIdTranslation:
    ...
    def test_empty_instances_keys_drops_all(self):
        ...
        out = to_daft_events(
            [pl_event(instances=["id_6131", "id_6127"])],
            ctx=VIDEO_CTX,
            instances_keys=[],
        )
        [ev] = out["events"]
    ...
    def test_multiple_events_share_one_index(self):
        ...
        out = to_daft_events(
            [
                pl_event(event_id="e1", instances=["id_7"]),
                pl_event(event_id="e2", instances=["id_42", "id_999"]),
            ],
            ctx=VIDEO_CTX,
            instances_keys=["car_7", "motorcycle_42"],
        )
...
class TestEventCaptionStripping:
    ...
    def test_caption_unchanged_without_instances_keys(self):
        ...
        ev_in = pl_event(event_caption="sedan {id: 6131} fabricated")
        out = to_daft_events([ev_in], ctx=VIDEO_CTX)
        [ev] = out["events"]
...
class TestVideoProseStripping:
    ...
    def test_strips_ungrounded_in_scene_description(self):
        out = to_daft_video(
            pl_video_dict(scene_description="Wide view; sedan {id: 6131} parked."),
            ctx=VIDEO_CTX,
            instances_keys=["car_7"],
        )
    ...
    def test_strips_ungrounded_in_event_summary(self):
        out = to_daft_video(
            pl_video_dict(event_summary="Motorcycle {id: 6127} hit by sedan."),
            ctx=VIDEO_CTX,
            instances_keys=["car_7"],
        )
...
def pl_image_dict(**overrides) -> dict:
    # Shape PL's image-input runner produces after `_enrich_image_with_probe`:
    ...
    base = {
        ...
        "source_video": "/input/frame.jpg",  # PL bookkeeping; must be stripped
...
class TestImage:
    def test_happy_path_only_allowed_keys(self):
        out = to_daft_image(pl_image_dict(), ctx=IMAGE_CTX)
        assert set(out.keys()) == {
            "version",
            "image_id",
            "format",
        ...
        assert out["version"] == DAFT_VERSION
        assert out["image_id"] == IMAGE_CTX.media_id
    ...
    def test_video_ctx_rejected(self):
        with pytest.raises(DaftConvertError, match="video SceneContext"):
            to_daft_image(pl_image_dict(), ctx=VIDEO_CTX)
    ...
    def test_format_enum_accepted(self, fmt):
        out = to_daft_image(pl_image_dict(format=fmt), ctx=IMAGE_CTX)
        assert out["format"] == fmt

    @pytest.mark.parametrize("bad_fmt", ["mp4", "gif", "tif", ""])
    def test_bad_format_rejected(self, bad_fmt):
        with pytest.raises(DaftConvertError, match="not in DAFT enum"):
            to_daft_image(pl_image_dict(format=bad_fmt), ctx=IMAGE_CTX)

    def test_height_width_accept_whole_floats(self):
        out = to_daft_image(pl_image_dict(height=1080.0, width=1920.0), ctx=IMAGE_CTX)
        assert out["height"] == 1080
        assert out["width"] == 1920

    def test_fractional_dim_rejected(self):
        with pytest.raises(DaftConvertError, match="height"):
            to_daft_image(pl_image_dict(height=1080.5), ctx=IMAGE_CTX)
    ...
    def test_image_id_overrides_anything_in_payload(self):
        ...
        out = to_daft_image(
            {**pl_image_dict(), "image_id": "whatever-the-vlm-said"},
            ctx=IMAGE_CTX,
        )
        assert out["image_id"] == IMAGE_CTX.media_id
