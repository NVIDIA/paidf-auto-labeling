...
- Use ONLY these headings (`## ...` and `### ...`) and NOTHING else: the required
  skeleton title `## Analyze Video Frames for Scene Evidence`, plus
  `## SCENE SUMMARY`, `## GLOBAL CONTEXT`, `## PER-QUESTION EVIDENCE`, and the
  per-question `### <id> <question>` blocks. Do NOT add any additional headings
  or sections.
